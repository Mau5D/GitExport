...
*      IMPORTING keys FOR CarParts~calculateOrderPrice.
    ...
    METHODS setInitialComponents FOR MODIFY
      IMPORTING keys FOR ACTION CarParts~setInitialComponents.
...
*
*        MESSAGE e004(zdar_rap_cars_msg) WITH <fs_carpart>-CarUuid INTO DATA(dummy).
*
*        APPEND VALUE #( %tky = Cars[ 1 ]-%tky
*                  %state_area        = 'VALIDATE_RECEIVE_DATE'
...
*     RESULT DATA(carparts).
    ...
    LOOP AT cars INTO DATA(car).
      ...
      SELECT * FROM zdar_carparts WHERE car_uuid = @car-Caruuid INTO TABLE @DATA(lt_temp).
      ...
      LOOP AT lt_temp ASSIGNING FIELD-SYMBOL(<fs_temp>).
        MODIFY ENTITIES OF zdar_i_cars IN LOCAL MODE
                ENTITY carparts
                    ...
                    #( (    CarpartsUuid = <fs_temp>-carparts_uuid
                            CarUuid = <fs_temp>-car_uuid
                            ManufacturerId = <fs_temp>-manufacturer_id
                            CreatedBy = sy-uname
    ...
    READ ENTITIES OF zdar_i_cars IN LOCAL MODE
         ...
         RESULT DATA(carparts).
*
*    "DELETE carparts WHERE CurrencyCode IS INITIAL.
*    LOOP AT carparts ASSIGNING FIELD-SYMBOL(<fs_carpart>).
*      SELECT * FROM zdar_manufactr WHERE supported_brand =
*endloop.
*
*
...
*
**    TRY.
**        lv_order-orderuuid = to_upper( cl_uuid_factory=>create_system_uuid( )->create_uuid_x16( ) ).
